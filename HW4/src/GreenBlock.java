...
public class GreenBlock extends Block {  // GreenBlock 클래스는 Block 클래스를 상속받아 구체적인 Green Block을 구현
    private static final int BLOCK_WIDTH = 50; // 각 블록의 너비를 정의하는 상수
 
    public GreenBlock(double x, double y) {  // 생성자: 초기 좌표 (x, y)와 블록 타입을 지정하여 GreenBlock을 초기화
        super(x, y,BlockType.GREEN_BLOCK);
    }
    ...
    public void init(double x, double y) { // rectangles 배열의 각 요소를 지정된 좌표와 크기로 초기화
        ...
        rectangles[2] = new Rectangle2D.Double(x + BLOCK_WIDTH * 2, y + BLOCK_WIDTH, BLOCK_WIDTH, BLOCK_WIDTH);
